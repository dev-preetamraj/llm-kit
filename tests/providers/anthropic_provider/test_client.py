from unittest.mock import AsyncMock, MagicMock, patch

import pytest
from pydantic import BaseModel

from llm_kit_pro.core.inputs import LLMFile
from llm_kit_pro.providers.anthropic.client import AnthropicClient
from llm_kit_pro.providers.anthropic.config import AnthropicConfig


@pytest.fixture
def mock_anthropic_client():
    with patch("anthropic.AsyncAnthropic") as mock:
        yield mock


@pytest.mark.asyncio
async def test_generate_text_without_files(mock_anthropic_client):
    # Setup mock response
    mock_response = MagicMock()
    mock_response.content = [MagicMock(text="Hello world")]

    mock_instance = mock_anthropic_client.return_value
    mock_instance.messages.create = AsyncMock(return_value=mock_response)

    client = AnthropicClient(AnthropicConfig(api_key="fake-key"))
    result = await client.generate_text("Say hello")

    assert result == "Hello world"
    mock_instance.messages.create.assert_called_once()
    _, kwargs = mock_instance.messages.create.call_args
    assert kwargs["messages"] == [
        {"role": "user", "content": [{"type": "text", "text": "Say hello"}]}
    ]


@pytest.mark.asyncio
async def test_generate_text_with_image_file(mock_anthropic_client):
    fake_image = LLMFile(
        content=b"fake-image-data",
        mime_type="image/png",
        filename="test.png",
    )

    mock_response = MagicMock()
    mock_response.content = [MagicMock(text="I see an image")]

    mock_instance = mock_anthropic_client.return_value
    mock_instance.messages.create = AsyncMock(return_value=mock_response)

    client = AnthropicClient(AnthropicConfig(api_key="fake-key"))
    result = await client.generate_text(
        "What is this?",
        files=[fake_image],
    )

    assert result == "I see an image"
    mock_instance.messages.create.assert_called_once()
    _, kwargs = mock_instance.messages.create.call_args

    content = kwargs["messages"][0]["content"]
    assert len(content) == 2
    assert content[0]["type"] == "text"
    assert content[1]["type"] == "image"
    assert content[1]["source"]["type"] == "base64"
    assert content[1]["source"]["media_type"] == "image/png"


@pytest.mark.asyncio
async def test_generate_json(mock_anthropic_client):
    class AmountSchema(BaseModel):
        amount: float

    expected_data = {"amount": 123.45}

    mock_response = MagicMock()
    # Anthropic returns tool_use block for JSON
    mock_tool_use = MagicMock()
    mock_tool_use.type = "tool_use"
    mock_tool_use.name = "format_output"
    mock_tool_use.input = expected_data
    mock_response.content = [mock_tool_use]

    mock_instance = mock_anthropic_client.return_value
    mock_instance.messages.create = AsyncMock(return_value=mock_response)

    client = AnthropicClient(AnthropicConfig(api_key="fake-key"))
    result = await client.generate_json(
        prompt="Extract amount",
        schema=AmountSchema,
    )

    assert result == expected_data
    mock_instance.messages.create.assert_called_once()
    _, kwargs = mock_instance.messages.create.call_args
    assert "tools" in kwargs
    assert kwargs["tool_choice"] == {"type": "tool", "name": "format_output"}
